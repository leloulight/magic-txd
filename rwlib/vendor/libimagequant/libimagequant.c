...
struct liq_attr {
    ...
    bool last_index_transparent, use_contrast_maps, use_dither_map, fast_palette, allow_sort;
    unsigned int speed;
...
LIQ_EXPORT LIQ_NONNULL void liq_set_allow_palette_sorting(liq_attr* attr, int allow)
{
    if (!CHECK_STRUCT_TYPE(attr, liq_attr)) return;

    attr->allow_sort = allow;
}
...
LIQ_EXPORT liq_attr* liq_attr_create_with_allocator(void* (*custom_malloc)(size_t), void (*custom_free)(void*))
{
    ...
    *attr = (liq_attr) {
        ...
        .allow_sort = true, // if false then palette indice must conform to fixed color input order
        .target_mse = 0,
        .max_mse = MAX_DIFF,
    };
