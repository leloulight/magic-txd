...
void pspNativeTextureTypeProvider::SerializeTexture( TextureBase *theTexture, PlatformTexture *nativeTex, BlockProvider& inputProvider ) const
{
    ...
    {
        const std::string& texName = theTexture->GetName();

        utils::writeStringChunkANSI( engineInterface, inputProvider, texName.c_str(), texName.size() );
    }
    {
        const std::string& maskName = theTexture->GetMaskName();

        utils::writeStringChunkANSI( engineInterface, inputProvider, maskName.c_str(), maskName.size() );
    }
...
)
{
    ...
    try
    {
        ...
        if ( srcPaletteType == PALETTE_NONE )
        {
            ...
            const void *colorSourceTexels = NULL;
...
{
    void *allocatedTexels = NULL;
    uint32 allocatedTexelsDataSize = 0;

    uint32 transRowAlignment = 0;

    const void *linearTransColors = NULL;
