        ...
        if ( this->blockMode == RWBLOCKMODE_READ )
        {
            ...
            if ( contextStream && contextStream->supportsSize() )
            {
                ...
                if ( virtualSize > 0 )
                {
                    ...
                    streamMemSlice_t::eIntersectionResult intResult = fileSpace.intersectWith( virtualSpace );

                    if ( intResult == streamMemSlice_t::INTERSECT_BORDER_END )
                    {
...
void BlockProvider::LeaveContext( void )
{
...
void BlockProvider::read_native( void *out_buf, size_t readCount ) throw( ... )
{
    Stream *contextStream = this->contextStream;
...
void BlockProvider::read( void *out_buf, size_t readCount ) throw( ... )
{
...
void BlockProvider::write_native( const void *in_buf, size_t writeCount ) throw( ... )
{
    Stream *contextStream = this->contextStream;
...
void BlockProvider::write( const void *in_buf, size_t writeCount ) throw( ... )
{
    if ( this->isInContext == false )
    {
        throw RwBlockException( "not in a block context" );
    ...
    if ( this->blockMode == RWBLOCKMODE_WRITE )
    {
        ...
        streamMemSlice_t::eIntersectionResult intResult = writeAccess.intersectWith( virtualSlice );

        if ( intResult == streamMemSlice_t::INTERSECT_BORDER_START ||
             intResult == streamMemSlice_t::INTERSECT_FLOATING_END ||
             intResult == streamMemSlice_t::INTERSECT_UNKNOWN )
        {
...
void BlockProvider::skip_native( size_t skipCount ) throw( ... )
{
    Stream *contextStream = this->contextStream;
...
void BlockProvider::skip( size_t skipCount ) throw( ... )
{
    if ( this->isInContext == false )
    {
        throw RwBlockException( "not in a block context" );
...
void BlockProvider::seek( int64 pos, eSeekMode mode ) throw( ... )
{
    if ( this->isInContext == false )
    {
        throw RwBlockException( "not in a block context" );
...
uint32 BlockProvider::getBlockID( void ) const throw( ... )
{
    if ( this->isInContext == false )
    {
        throw RwBlockException( "not in a block context" );
...
int64 BlockProvider::getBlockLength( void ) const throw( ... )
{
    if ( this->isInContext == false )
    {
        throw RwBlockException( "not in a block context" );
...
LibraryVersion BlockProvider::getBlockVersion( void ) const throw( ... )
{
    if ( this->isInContext == false )
    {
        throw RwBlockException( "not in a block context" );
...
void BlockProvider::verifyLocalStreamAccess( const streamMemSlice_t& requestedMemory ) const
{
    if ( requestedMemory.GetSliceSize() > 0 )
    {
        ...
        if ( this->ignoreBlockRegions == false )
        {
            ...
            streamMemSlice_t::eIntersectionResult intResult = requestedMemory.intersectWith( virtualSlice );

            if ( intResult != streamMemSlice_t::INTERSECT_EQUAL &&
                 intResult != streamMemSlice_t::INTERSECT_INSIDE )
            {
        ...
        if ( Stream *contextStream = this->contextStream )
        {
            if ( contextStream->supportsSize() == true )
            {
                ...
                streamMemSlice_t::eIntersectionResult intResult = requestedMemory.intersectWith( fileSlice );
