...
inline void ErrorIfFailed( bool success, const char *errorMsg )
{
    if ( !success )
    {
        throw RwException( errorMsg );
    }
}
...
ComPtr <ID3D12RootSignature> createRootSignature( d3d12DriverInterface *env, d3d12DriverInterface::d3d12NativeDriver *driver, const gfxGraphicsState& psoState )
{
    ...
    ComPtr <ID3D12RootSignature> rootSigOut = NULL;
    ...
    D3D12_ROOT_PARAMETER *rootParams = NULL;
    {
        if ( numRootParams != 0 )
        {
    ...
    std::vector <D3D12_DESCRIPTOR_RANGE*> descriptorRangePtrs;
        ...
        try
        {
            ...
            for ( uint32 n = 0; n < numRootParams; n++ )
            {
                ...
                ErrorIfFailed(
                    ...
                    "failed to get mapping for root parameter"
                );
                ...
                if ( paramType == gfxRootParameterType::DESCRIPTOR_TABLE )
                {
                    ...
                    for ( uint32 range_iter = 0; range_iter < numRanges; range_iter++ )
                    {
                        ...
                        ErrorIfFailed(
                            ...
                            "failed to get descriptor range type"
                        );
                    ...
                    descriptorRangePtrs.push_back( descriptorRanges );
                }
                ...
                ErrorIfFailed(
                    ...
                    "failed to get root parameter shader visibility"
                );
            }
            ...
            for ( uint32 n = 0; n < numStaticSamplers; n++ )
            {
                ...
                ErrorIfFailed(
                    ...
                    "failed to get static sampler shader visibility"
                );
            }
            ...
            rootSig.Flags = D3D12_ROOT_SIGNATURE_FLAG_NONE;
            ...
            if ( !SUCCEEDED( serializeSuccess ) )
            {
                ...
                std::string errorMsg = "unknown error while serializing root signature";

                if ( ID3DBlob *blob = errorBlob.Get() )
                {
                    errorMsg = std::string( (const char*)blob->GetBufferPointer(), blob->GetBufferSize() );
                }
                
                throw RwException( std::move( errorMsg ) );
            }
            ...
            {
                throw RwException( "failed to create root signature" );
            }
        }
        catch( ... )
        {
            for ( size_t n = 0; n < descriptorRangePtrs.size(); n++ )
            {
...
d3d12DriverInterface::d3d12PipelineStateObject::d3d12PipelineStateObject( d3d12DriverInterface *env, Interface *engineInterface, d3d12NativeDriver *driver, const gfxGraphicsState& psoState )
{
    ...
    stateDesc.StreamOutput.pSODeclaration = NULL;
    stateDesc.StreamOutput.NumEntries = 0;
    stateDesc.StreamOutput.pBufferStrides = NULL;
    ...
    // Rasterizer state.
    {
        ...
        ErrorIfFailed(
            ...
            "failed to get rasterizer state fill mode"
        );
        ...
        ErrorIfFailed(
            ...
            "failed to get rasterizer state cull mode"
        );
    ...
    // Depth stencil state.
    {
        ...
        ErrorIfFailed(
            ...
            "failed to get depth stencil depth func"
        );
    ...
    try
    {
        // Input layout.
        {
            ...
            if ( numInputElems != 0 )
            {
                ...
                if ( !inputElemArray )
                {
                    throw RwException( "failed to allocate input element array" );
                }
                ...
                for ( uint32 n = 0; n < numInputElems; n++ )
                {
                    ...
                    if ( inputElem.SemanticName == NULL )
                    {
                        throw RwException( "failed to map vertex attribute semantic" );
                    }
                    ...
                    ErrorIfFailed(
                        ...
                        "failed to map vertex attribute to DXGI_FORMAT"
                    );
        ...
        ErrorIfFailed(
            ...
            "failed to map primitive topology type"
        );
        ...
        for ( uint32 n = 0; n < _countof(psoState.renderTargetFormats); n++ )
        {
            ErrorIfFailed(
                ...
                "failed to map render target view raster format"
            );
        }

        ErrorIfFailed(
            getD3D12RasterFormat( psoState.depthStencilFormat, stateDesc.DSVFormat ),
            "failed to map depth stencil view raster format"
        );
        ...
        stateDesc.CachedPSO.pCachedBlob = NULL;
        ...
        {
            throw RwException( "failed to create graphics pipeline state object" );
        }
    }
    catch( ... )
    {
