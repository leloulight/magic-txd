...
struct jpegImagingExtension : public imagingFormatExtension
{
    ...
    static noreturn_t jpeg_onFatalError( j_common_ptr cinfo )
    {
        ...
        throw RwException( std::string( "JPEG fatal error: " ) + msgbuf );
    }
    ...
    static void jpeg_outputMessage( j_common_ptr cinfo )
    {
        ...
        // Print it as warning.
        errorMgr->engineInterface->PushWarning( std::string( "JPEG: " ) + msgbuf );
    }
    ...
    static void skip_input_data( j_decompress_ptr cinfo, long num_bytes )
    {
        if ( num_bytes < 0 )
        {
            throw RwException( "invalid byte skip count in JPEG deserialization" );
        }
    ...
    void DeserializeImage( Interface *engineInterface, Stream *inputStream, imagingLayerTraversal& outputPixels ) const override
    {
        ...
        try
        {
            ...
            if ( rdbuf == NULL )
            {
                throw RwException( "failed to allocate read buffer for JPEG deserialization" );
            }
            ...
            try
            {
                ...
                {
                    throw RwException( "unknown JPEG format in deserialization" );
                }
                ...
                if ( dstTexels == NULL )
                {
                    throw RwException( "failed to allocate destionation pixel buffer in JPEG deserialization" );
                }
                ...
                try
                {
                    ...
                    void **scanlineArray = (void**)engineInterface->MemAllocate( height * sizeof(void*) );

                    if ( scanlineArray == NULL )
                    {
                        throw RwException( "failed to allocate scanline array for JPEG deserialization" );
                    }
                    ...
                    try
                    {
                        ...
                        {
                            scanlineArray[ row ] = getTexelDataRow( dstTexels, dstRowSize, row );
                        }
    ...
    void SerializeImage( Interface *engineInterface, Stream *outputStream, const imagingLayerTraversal& inputPixels ) const override
    {
        ...
        {
            throw RwException( "cannot accept compressed pixels in JPEG serialization routine" );
        }
        ...
        try
        {
            ...
            void *writebuf = engineInterface->MemAllocate( writebuf_size );

            if ( writebuf == NULL )
            {
                throw RwException( "failed to allocate I/O buffer for JPEG serialization" );
            }
            ...
            try
            {
                ...
                {
                    throw RwException( "unsupported raster format in JPEG serialization" );
                }
                ...
                try
                {
                    ...
                    void **rowpointers = (void**)engineInterface->MemAllocate( sizeof(void*) * height );

                    if ( rowpointers == NULL )
                    {
                        throw RwException( "failed to allocate row pointer buffer during JPEG serialization" );
                    }
                    ...
                    try
                    {
                        ...
                        for ( uint32 row = 0; row < height; row++ )
                        {
                            rowpointers[ row ] = getTexelDataRow( dstTexels, dstRowSize, row );
                        }
