...
bool Config::GetConfigLine( CFile *stream, std::string& configLineOut )
{
    ...
    bool getLine = stream->GetString( configLine );

    if ( getLine )
    {
        // We can hack here. I guess.
        char *cfgLineBuf = (char*)configLine.c_str();
        int cfgLineCount = (int)configLine.size();
        ...
        if ( cfgLineCount > 0 )
        {
            ...
            configLineOut = std::string( cfgStart, configLine.size() - ( cfgStart - cfgLineBuf ) );
        }
        else
        {
            configLineOut.clear();
